  ...
  const handleUpdateSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'settings', 'main'), {
        ...
        heroBackground: settingsForm.heroBackground,
      ...
      {activeTab === 'settings' && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
          ...
          <form onSubmit={handleUpdateSettings} className="space-y-6 max-w-2xl">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700">URL Gambar Latar Belakang Hero</label>
              <div className="flex gap-4 items-start">
                <div className="flex-grow">
                  ...
                  />
                  <p className="text-xs text-slate-500 mt-1">Gunakan URL gambar untuk latar belakang bagian atas halaman beranda.</p>
                </div>
                {settingsForm.heroBackground && (
                  <div className="w-24 h-16 rounded-lg overflow-hidden border border-slate-200 bg-slate-100 flex-shrink-0">
                    <img 
                      src={getDirectImageUrl(settingsForm.heroBackground)} 
                      alt="Preview" 
                      className="w-full h-full object-cover"
                      referrerPolicy="no-referrer"
                      onError={(e) => (e.currentTarget.src = 'https://picsum.photos/seed/error/200/150')}
                    />
                  </div>
                )}
              </div>
